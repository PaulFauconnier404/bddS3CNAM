...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    ...
    <a href="#" class="d-none d-sm-inline-block btn btn-primary btn-icon-split shadow-sm" data-toggle="modal" data-target="#createModal">
                ...
                <tbody>
                    ...
                    foreach ($tab as $p) {
                        echo '
                            <tr>
                                <td>
                                    <a href="index.php?controller=' . static::$object . '&action=read&id=' . rawurldecode($p['id']) . '" class="btn btn-info btn-circle btn-sm">
                                        <i class="fas fa-info-circle"></i>
                                    </a>
                                    <a href="index.php?controller=' . static::$object . '&action=deleted&id=' . rawurldecode($p['id']) . '" class="btn btn-danger btn-circle btn-sm">
                                        <i class="fas fa-trash"></i>
                                    </a>
                                </td>
                                <td>' . htmlspecialchars($p['depart']) . '</td>
                            ...
                            <div class="p-5">
                                <form method="get" action="index.php" class="user" id="<?php echo static::$object ?>_create">
                                    ...
                                    <div class="form-group">
                                        <input type="text" class="form-control form-control-user" name="depart" id="depart_id" placeholder="Date de départ" required>
                                    </div>
                                    <div class="form-group">
                                        <select class="custom-select form-control" name="idconducteur">
                                            <option selected value="-1">Sélectionner un conducteur</option>
                                            <?php
                                            foreach ($tabConducteur as $conducteur) {
                                                $personne = (ModelParcours::selectNomPersonne($conducteur->get('idpersonne')));
                                                echo '<option value="' . $conducteur->get('id') . '">' . $personne[0]['nom'] . '</option>';
                                            }
                                            ?>
                                        </select>
                                    </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <?php
                    echo '
                            <p>Date de départ : ' . htmlspecialchars($datas['depart']) . '</p>
                            <p>Conducteur : ' . htmlspecialchars($datas['nom']) . '</p>
                            <p>Ligne : ' . htmlspecialchars($datas['numero']) . '</p>
                        ';
                    ?>
                </div>
